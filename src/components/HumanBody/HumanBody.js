...
import { BODY_CARDS } from "../../data/humanBodyCategories";
...
function renderCards(section) {
  ...
  if (section === "legs") {
    const items = BODY_CARDS.legs;
    const left  = items.slice(0, 2);
    const right = items.slice(2);

    return (
      <div className="legs-cards" id="category-cards" aria-live="polite">
        <div className="left-legs-cards">
          {left.map((c) => (
            <Card key={c.title} title={c.title} link={c.link} img={c.img} />
          ))}
        </div>
  ...
  return (
    <div className={`${section}-cards`} id="category-cards" aria-live="polite">
      {BODY_CARDS[section].map((c) => (
        <Card key={c.title} title={c.title} link={c.link} img={c.img} />
      ))}
    </div>
  );
...
function HumanBody() {
  ...
  return (
    <div className="main-body">
      {/* Rendu des cartes selon la section actuellement ouverte */}
      {openSection && renderCards(openSection)}
      ...
      <div className="HumanBody">
        <div>
          ...
          <svg
            role="button"
            tabIndex={0}
            aria-pressed={openSection === "head"}
            aria-label="Ouvrir la catégorie Tête/Cou"
            onClick={() => toggleSection("head")}
            onKeyDown={(e) => onKeyActivate(e, "head")}
            data-position="head"
            className="head"
            xmlns="http://www.w3.org/2000/svg" // namespace SVG (standard W3C)
            width="56.594"
            height="95.031"
          ...
          <svg
            role="button"
            tabIndex={0}
            aria-pressed={openSection === "shoulder"}
            aria-label="Ouvrir la catégorie Épaules"
            onClick={() => toggleSection("shoulder")}
            onKeyDown={(e) => onKeyActivate(e, "shoulder")}
          ...
          <svg
            role="button"
            tabIndex={0}
            aria-pressed={openSection === "arm"}
            aria-label="Ouvrir la catégorie Bras/Avant-bras"
            onClick={() => toggleSection("arm")}
            onKeyDown={(e) => onKeyActivate(e, "arm")}
            data-position="arm"
            className="arm"
            xmlns="http://www.w3.org/2000/svg"
            width="156.344"
          ...
          <svg
            role="button"
            tabIndex={0}
            aria-pressed={openSection === "cheast"}
            aria-label="Ouvrir la catégorie Poitrine/Dos"
            onClick={() => toggleSection("cheast")}
            onKeyDown={(e) => onKeyActivate(e, "cheast")}
          ...
          <svg
            role="button"
            tabIndex={0}
            aria-pressed={openSection === "stomach"}
            aria-label="Ouvrir la catégorie Ventre/Hanche"
            onClick={() => toggleSection("stomach")}
            onKeyDown={(e) => onKeyActivate(e, "stomach")}
          ...
          <svg
            role="button"
            tabIndex={0}
            aria-pressed={openSection === "legs"}
            aria-label="Ouvrir la catégorie Jambes/Pieds"
            onClick={() => toggleSection("legs")}
            onKeyDown={(e) => onKeyActivate(e, "legs")}
            data-position="legs"
            className="legs"
            xmlns="http://www.w3.org/2000/svg"
            width="93.626"
